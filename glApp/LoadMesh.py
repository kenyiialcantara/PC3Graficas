    ...
    def load_drawing(self, filename):
        ...
        with open(filename) as fp:
            ...
            while line:
                ...
                if line[:2] == "f ":
                    print(line[2:],'a')
                    arrT = [value for value in line[2:].split()]

                    if len(arrT) == 3:
                        t1, t2, t3 = [value for value in line[2:].split()]
                        triangles.append([int(0 if value == '' else value) for value in t1.split('/')][0]-1)
                        triangles.append([int(0 if value == '' else value) for value in t2.split('/')][0]-1)
                        triangles.append([int(0 if value == '' else value) for value in t3.split('/')][0]-1)
                    elif len(arrT) == 4:
                        t1, t2, t3, t4 = [value for value in line[2:].split()]
                        triangles.append([int(0 if value == '' else value) for value in t1.split('/')][0] - 1)
                        triangles.append([int(0 if value == '' else value) for value in t2.split('/')][0] - 1)
                        triangles.append([int(0 if value == '' else value) for value in t3.split('/')][0] - 1)
                        # triangles.append([int(value) for value in t4.split('/')][0] - 1)
